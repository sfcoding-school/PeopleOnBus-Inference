...
check(C,A,Z,X,Y,L):-J#=X+1,checkpoint(J,C),not_present(J,Y,L),V#=X+1,constraint_prev(V,Y,Z,L,Z,L),W#=X-1,constraint_next(W,Y,Z,L,Z,L),A#=0.
check(C,A,Z,X,Y,L):-J#=X-1,checkpoint(J,C),not_present(J,Y,L),V#=X+1,constraint_prev(V,Y,Z,L,Z,L),W#=X-1,constraint_next(W,Y,Z,L,Z,L),A#=0.
check(_,_,_,_,_,_).

constraint_next(_,_,[],[],_,_).
constraint_next(X,Y,[A|_],[[X,Y]|_],P,T):-A#=0,V=X+1,constraint_next(V,Y,P,T,P,T).
constraint_next(X,Y,[_|R],[[_,_]|L],P,T):-constraint_next(X,Y,R,L,P,T).

constraint_prev(_,_,[],[],_,_).
constraint_prev(X,Y,[A|_],[[X,Y]|_],P,T):-A#=0,V=X-1,constraint_next(V,Y,P,T,P,T).
